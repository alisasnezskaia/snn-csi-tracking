"""Leave-activity-out CV for ConvSNNPresenceClassifier -- the genuinely
spiking version of ImageCNNPresenceClassifier (see conversation): keeps
the same real-2D-conv-across-time advantage that made the plain-CNN test
the best result of the night (AUROC=0.650+/-0.135), but replaces the ReLU
classifier with actual LIF neurons + delta encoding, matching this
project's actual efficiency thesis.

Same window-level presence labels, same 4-fold leave-activity-out
protocol, same best-of-best preprocessing (relative motion, energy
amplitude normalization) as train_image_cnn_presence.py -- directly
comparable to that script's summary and to
train_presence_position_cv.py's relative-motion baseline
(presence_acc=0.627+/-0.066, AUROC=0.542+/-0.061).

Run:
    .venv/bin/python scripts/train_conv_snn_presence.py
"""
...
from snn_csi_tracking.models.conv_snn_presence import ConvSNNPresenceClassifier
...
def run_fold(X, labels, groups, test_activity: str, val_activity: str) -> dict:
    ...
    model = ConvSNNPresenceClassifier(num_channels=X.shape[1], num_subcarriers=X.shape[2], t_win=X.shape[3]).to(DEVICE)
...
def main():
    ...
    for i, test_activity in enumerate(ROTATION):
        ...
        print(f"\n=== conv-SNN fold {i+1}/{len(ROTATION)}: test={test_activity}, val={val_activity} ===")
        t0 = time.time()
    ...
    print(f"\n=== conv-SNN cross-validation summary ({len(ROTATION)} folds) ===")
    ...
    print("(compare against image-CNN (non-spiking): presence_acc=0.731+/-0.196, AUROC=0.650+/-0.135)")
